...
import { HUB_IO_DATA_ACTIONS } from '../actions';
...
export class HubIoDataEffects {
    public readonly subscribeToPortValues$ = createEffect(() => {
        return this.actions$.pipe(
            ...
            mergeMap(async (action) => {
                await this.hubStorage.get(action.hubId).ports.setPortInputFormat(action.portId, action.modeId, true);
    ...
    public readonly unsubscribeFromPortValues$ = createEffect(() => {
        return this.actions$.pipe(
            ...
            mergeMap(([ action, data ]) => this.hubStorage.get(action.hubId).ports.setPortInputFormat(action.portId, data!.modeId, false))
        );
    ...
    public listenPortValues$ = createEffect(() => {
        return this.actions$.pipe(
            ...
            mergeMap((action) => {
                return this.hubStorage.get(action.hubId).ports.listenPortValueUpdates$(action.portId).pipe(
                    takeUntil(this.actions$.pipe(ofType(HUB_IO_DATA_ACTIONS.unsubscribeFromPortValues))),
