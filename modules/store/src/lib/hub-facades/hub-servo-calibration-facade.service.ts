...
export class HubServoCalibrationFacadeService {
  ...
  private doCalibration(hubId: string, portId: number, speed: number, power: number, calibrationRuns: number): Observable<CalibrationResultFinished> {
    return this.getPreCalibrationData(hubId, portId).pipe(
      switchMap(({ startAbsolutePosition, startRelativePosition, positionModeId }) => {
        return this.getServoRange(hubId, portId, positionModeId, speed, power, calibrationRuns).pipe(
          map((result) => {
            ...
            if (
              calibrationResult.servoRange > this.appConfig.servo.maxServoRange ||
      ...
      switchMap((data) =>
        this.finalizeCalibration(hubId, portId, speed, power, data.arcCenterPosition).pipe(
          map(() => {
  ...
  private probeDirectionLimit(hub: IHub, portId: number, speed: number, power: number, maxDistance: number, positionModeId: number): Observable<number> {
    return new Observable((subscriber) => {
      ...
      const positionSubscription = hub.ports
        .getPortValue(portId, positionModeId)
        .pipe(
          map((value) => ValueTransformers.position.fromRawValue(value)),
          switchMap((startPosition) =>
            hub.ports.portValueChanges(portId, positionModeId, this.calibrationPositionMinimumThreshold).pipe(
              map((value) => ValueTransformers.position.fromRawValue(value)),
  ...
  private getHub(hubId: string): IHub {
    const hub = this.hubStorage.get(hubId);
    if (!hub) {
