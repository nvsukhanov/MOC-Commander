...
export const CONTROLLER_CONNECTION_SELECTORS = {
  ...
  selectGamepadConnections: createSelector(
    ...
    (allConnections, controllerEntities, settings) => {
      return allConnections
        .filter((connection) => controllerEntities[connection.controllerId]?.controllerType === ControllerType.Gamepad)
        .map((connection) => ({
  ...
  selectKeyboardConnection: createSelector(SELECT_ALL, (allConnections) =>
    allConnections.find((connection) => connection.controllerType === ControllerType.Keyboard),
  ),
} as const;
