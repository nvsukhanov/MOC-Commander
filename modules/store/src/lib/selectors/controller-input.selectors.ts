...
const SELECT_ALL = createSelector(CONTROLLER_INPUT_FEATURE.selectControllerInputState, CONTROLLER_INPUT_ENTITY_ADAPTER_SELECTORS.selectAll);

export const CONTROLLER_INPUT_SELECTORS = {
  selectAll: SELECT_ALL,
  selectEntities: createSelector(CONTROLLER_INPUT_FEATURE.selectControllerInputState, CONTROLLER_INPUT_ENTITY_ADAPTER_SELECTORS.selectEntities),
  selectRawValueById: (id: string) => createSelector(CONTROLLER_INPUT_SELECTORS.selectEntities, (entities) => entities[id]?.rawValue ?? 0),
  selectValueById: (id: string) =>
    createSelector(CONTROLLER_INPUT_SELECTORS.selectEntities, CONTROLLER_SETTINGS_SELECTORS.selectEntities, (inputEntities, settingsEntities) => {
  ...
  selectIsActivatedById: (id: string) =>
    createSelector(CONTROLLER_INPUT_SELECTORS.selectEntities, CONTROLLER_SETTINGS_SELECTORS.selectEntities, (inputEntities, controllerSettingsEntities) => {
      const entity = inputEntities[id];
      if (!entity) {
        return false;
      }
  ...
  selectFirstActivated: createSelector(SELECT_ALL, CONTROLLER_SETTINGS_SELECTORS.selectEntities, (inputsList, controllerSettingsEntities) => {
  ...
  isCapturing: createSelector(CONTROLLER_INPUT_FEATURE.selectListenersCount, (listenersCount) => listenersCount > 0),
