...
export const CONTROL_SCHEME_WIDGETS_DATA_SELECTORS = {
  selectById: (widgetId: number) =>
    createSelector(CONTROL_SCHEME_WIDGET_DATA_FEATURE.selectControlSchemeWidgetsDataState, (state) => state.widgetsData[widgetId]),
  selectWidgetTiltData: ({ id, hubId, portId }: { id: number; hubId: string; portId: number }) =>
    createSelector(
      ...
      (widgetData, attachedIoProps) => {
        if (widgetData?.widgetType === WidgetType.Yaw || widgetData?.widgetType === WidgetType.Pitch || widgetData?.widgetType === WidgetType.Roll) {
