...
export class HubAttachedIosStateEffects {
  public readonly getMotorEncoderOffset$ = createEffect(() => {
    return this.actions.pipe(
      ...
      mergeMap((action) =>
        this.getModeIdForModeName(action.io, PortModeName.position).pipe(
          concatWith(this.getModeIdForModeName(action.io, PortModeName.absolutePosition)),
          bufferCount(2),
      ...
      mergeMap(({ positionModeId, absolutePositionModeId, action }) => {
        return (
          this.hubStorage
            ...
            .ports.getPortValue(action.io.portId, positionModeId!, ValueTransformers.position)
            .pipe(
              concatWith(
                this.hubStorage
                  .get(action.io.hubId)
  ...
  private getModeIdForModeName(io: AttachedIoModel, portModeName: PortModeName): Observable<number | null> {
    return this.store
      .select(ATTACHED_IO_PORT_MODE_INFO_SELECTORS.selectModeIdForIoAndPortModeName(io, portModeName))
      .pipe(
