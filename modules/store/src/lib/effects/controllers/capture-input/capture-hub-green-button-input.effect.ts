...
const HUB_INPUT_READ_SELECTOR = createSelector(
  ...
  (hubs, hubRuntimeIds, settings) => {
    ...
    return hubs
      .filter((hub) => hubRuntimeIdsSet.has(hub.hubId))
      .map((hub) => hub.hubId)
      .filter((hub) => {
...
function readHubsGreenButtons(store: Store, hubStorage: HubStorageService): Observable<Action> {
...
export const CAPTURE_HUB_GREEN_BUTTON_INPUT = createEffect(
  (store: Store = inject(Store), hubStorage: HubStorageService = inject(HubStorageService)) => {
    return store
      ...
      .pipe(switchMap((isCapturing) => (isCapturing ? readHubsGreenButtons(store, hubStorage) : NEVER)));
  },
  { functional: true },
);
