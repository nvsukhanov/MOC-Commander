  ...
  (
    ...
    return actions$.pipe(
      ...
      map(([, knownKeyboard]) => {
        ...
        if (knownKeyboard) {
          return CONTROLLERS_ACTIONS.keyboardConnected({ profileUid });
        }

        return CONTROLLERS_ACTIONS.keyboardDiscovered({
          profileUid,
          defaultSettings: controllerProfile.getDefaultSettings(),
        });
      }),
      take(1),
    );
