...
function getUniqueConnectedHubPorts(
  ...
  for (const binding of bindings) {
    ...
    if (
      knownHubPortIds.has(ioIds) ||
      !connectedHubIds.includes(binding.hubId) ||
      !attachedIos[attachedIosIdFn(binding)]
    ) {
      continue;
    }
...
function terminateScheme(
  ...
  return of(schemeModel).pipe(
    ...
    map(([scheme, connectedHubIds, attachedIos]) =>
      getUniqueConnectedHubPorts(scheme.bindings, connectedHubIds, attachedIos),
    ),
    map((uniquePortIds) => {
    ...
    map((cleanupTasks) =>
      cleanupTasks.filter((cleanupTask: PortCommandTask | null): cleanupTask is PortCommandTask => !!cleanupTask),
    ),
...
export const STOP_SCHEME_EFFECT = createEffect(
  (
    ...
    return actions.pipe(
      ...
      mergeMap((runningScheme) =>
        terminateScheme(runningScheme, store, taskFactory, hubStorage, taskRunner, appConfig.schemeStartStopTimeoutMs),
      ),
