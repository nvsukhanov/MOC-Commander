...
export function controllerInputIdFn(
  idArgs:
    | {
        controllerId: string;
        inputId: string;
        inputType: ControllerInputType.Button | ControllerInputType.Axis | ControllerInputType.Trigger;
      }
    | {
        controllerId: string;
        inputId: string;
...
export const CONTROLLER_INPUT_FEATURE = createFeature({
  ...
  reducer: createReducer(
    ...
    on(CONTROLLER_INPUT_ACTIONS.requestInputCapture, (state): IControllerInputState => {
      return {
        ...state,
        listenersCount: state.listenersCount + 1,
      };
    }),
    on(CONTROLLER_INPUT_ACTIONS.releaseInputCapture, (state): IControllerInputState => {
      ...
      const nextState = nextListenersCount === 0 ? CONTROLLER_INPUT_ENTITY_ADAPTER.removeAll(state) : state;
      return {
        ...nextState,
