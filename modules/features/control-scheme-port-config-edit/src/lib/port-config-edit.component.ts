...
import { PORT_CONFIG_EDIT_SELECTORS } from './port-config-edit.selectors';
import { PortConfigEditViewModel } from './port-config-edit-view-model';

@Component({
    standalone: true,
    selector: 'feat-control-scheme-port-config-edit',
    templateUrl: './port-config-edit.component.html',
    styleUrls: [ './port-config-edit.component.scss' ],
    imports: [
        NgIf,
        PushPipe,
        JsonPipe,
...
export class PortConfigEditComponent implements OnInit, OnDestroy {
    public readonly portConfig$: Observable<PortConfigEditViewModel | null> = this.store.select(PORT_CONFIG_EDIT_SELECTORS.selectPortConfig);
    ...
    public ngOnInit(): void {
        ...
        const pageTitle$ = this.store.select(PORT_CONFIG_EDIT_SELECTORS.selectPortConfig).pipe(
