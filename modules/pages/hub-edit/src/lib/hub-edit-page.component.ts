...
export class HubEditPageComponent implements OnInit, OnDestroy, IUnsavedChangesComponent {
  ...
  public readonly cancelPath$: Observable<string[]> = this.store
    .select(ROUTER_SELECTORS.selectCurrentlyEditedHubId)
    .pipe(map((id) => (id !== null ? this.routesBuilderService.hubView(id) : [])));
  ...
  public onSave(hubId: string): void {
