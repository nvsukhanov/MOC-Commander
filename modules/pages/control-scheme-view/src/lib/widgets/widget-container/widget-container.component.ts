...
export class WidgetContainerComponent implements OnDestroy {
  ...
  constructor(
    @Inject(CONTROL_SCHEME_WIDGET_COMPONENT_FACTORY)
    private readonly widgetFactory: IControlSchemeWidgetComponentFactory<WidgetType>,
  ) {}
  ...
  @Input()
  public set config(config: WidgetConfigModel) {
  ...
  private updateSpans(config: WidgetConfigModel): void {
