...
export const HUB_LIST_ITEM_SELECTORS = {
  selectIsHubKnown: (hubId: string) => createSelector(HUBS_SELECTORS.selectHub(hubId), (hub): boolean => !!hub),
  selectHubName: (hubId: string) => createSelector(HUBS_SELECTORS.selectHub(hubId), (hub): string => hub?.name ?? ''),
  ...
  selectControlSchemeHubPorts: (controlSchemeName: string, hubId: string) =>
