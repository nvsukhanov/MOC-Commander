...
@Component({
  ...
  imports: [MatInputModule, ReactiveFormsModule, ValidationMessagesDirective, TranslocoPipe],
...
export class VoltageSensorWidgetSettingsComponent implements OnDestroy {
  ...
  @Input()
  public set config(config: VoltageWidgetConfigModel | undefined) {
    ...
    if (!this.form.controls.title.valid) {
      this.translocoService
        .selectTranslate('controlScheme.widgets.voltage.defaultName')
        .pipe(take(1))
        .subscribe((name) => {
