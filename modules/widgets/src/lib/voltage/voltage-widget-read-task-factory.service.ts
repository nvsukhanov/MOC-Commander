...
export class VoltageWidgetReadTaskFactoryService implements IWidgetReadTaskFactory<VoltageWidgetConfigModel> {
  ...
  public createReadTask(config: VoltageWidgetConfigModel): Observable<{
    widgetId: number;
    ...
    return this.store.select(HUBS_SELECTORS.selectHub(config.hubId)).pipe(
      ...
      switchMap(({ hubType }) => {
        ...
        return hub.ports
          .getPortValue(config.portId, config.modeId, ValueTransformers.voltage(hubType))
          .pipe(concatWith(hub.ports.portValueChanges(config.portId, config.modeId, config.valueChangeThreshold, ValueTransformers.voltage(hubType))));
      }),
