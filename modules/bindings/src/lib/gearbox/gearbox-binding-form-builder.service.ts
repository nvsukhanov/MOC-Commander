...
export class GearboxBindingFormBuilderService implements IBindingFormBuilder<GearboxBindingForm> {
  ...
  public build(): GearboxBindingForm {
    return this.formBuilder.group({
      ...
      angles: this.formBuilder.array<FormControl<number>>([this.commonFormControlsBuilder.angleControl(0)], {
  ...
  public patchForm(form: GearboxBindingForm, binding: ControlSchemeGearboxBinding): void {
    form.patchValue(binding);
    form.controls.angles.clear();
    binding.angles.forEach((angle) => form.controls.angles.push(this.commonFormControlsBuilder.angleControl(angle)));
