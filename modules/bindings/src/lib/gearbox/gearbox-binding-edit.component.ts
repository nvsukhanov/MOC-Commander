...
export class GearboxBindingEditComponent implements IBindingsDetailsEditComponent<GearboxBindingForm>, OnDestroy {
  ...
  public setForm(form: GearboxBindingForm): void {
    this._form = form;
    ...
    this._canRequestPortValue$ = form.controls.hubId.valueChanges.pipe(
      ...
      switchMap(() => {
        ...
        return this.store.select(
          BINDING_EDIT_COMMON_SELECTORS.canRequestPortValue({
  ...
  public onSetMotorPosition(levelIndex: number): void {
    this.setMotorPositionSubscription?.unsubscribe();
  ...
  public onPortAbsolutePositionRequest(levelIndex: number): void {
    this.portRequestSubscription?.unsubscribe();
    ...
    this.portRequestSubscription = this.hubFacade
      .getMotorAbsolutePosition(this.form.controls.hubId.value, this.form.controls.portId.value)
      .pipe(take(1))
      .subscribe((position) => {
  ...
  public addNextAngleLevel(): void {
    ...
    this.form.controls.angles.insert(0, this.commonFormControlBuilder.angleControl(0));
    this.form.controls.initialLevelIndex.setValue(this.form.controls.initialLevelIndex.value + 1);
  ...
  public addPrevAngleLevel(): void {
    ...
    this.form.controls.angles.push(this.commonFormControlBuilder.angleControl(0));
    this.form.controls.angles.markAsDirty();
    this.form.updateValueAndValidity();
  }

  public removeAngleLevel(index: number): void {
    if (!this.form) {
      return;
    }
