...
export class GearboxBindingInputExtractorService implements ITasksInputExtractor<ControlSchemeBindingType.Gearbox> {
    ...
    public extractInputs(
        ...
        const nextGear$ = this.inputExtractorService.extractInputResult(
            binding,
            globalInput$,
            controllersSettings$,
        ...
        const prevGear$ = this.inputExtractorService.extractInputResult(
            binding,
            globalInput$,
            controllersSettings$,
        ...
        const reset$ = this.inputExtractorService.extractInputResult(
            ...
            globalInput$,
            controllersSettings$,
            binding.inputs[GearboxBindingInputAction.Reset]
        ).pipe(
            distinctUntilIsActivatedChanged()
        );
