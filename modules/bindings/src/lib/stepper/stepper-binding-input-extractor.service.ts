...
export class StepperBindingInputExtractorService implements ITasksInputExtractor<ControlSchemeBindingType.Stepper> {
  ...
  public extractInputs(
    ...
    const cw$ = this.inputExtractorService
      .extractInputResult(binding, globalInput$, controllersSettings$, binding.inputs[StepperBindingInputAction.Cw])
      .pipe(distinctUntilIsActivatedChanged());
    const ccw$ = this.inputExtractorService
      .extractInputResult(binding, globalInput$, controllersSettings$, binding.inputs[StepperBindingInputAction.Ccw])
      .pipe(distinctUntilIsActivatedChanged());
