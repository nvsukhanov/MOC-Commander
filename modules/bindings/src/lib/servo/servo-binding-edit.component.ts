...
export class ServoBindingEditComponent implements IBindingsDetailsEditComponent<ServoBindingForm>, OnDestroy {
  ...
  public calibrate(): void {
    ...
    this.matDialog
      .open(ServoCalibrationDialogComponent, {
        data: {
      ...
      .afterClosed()
      .subscribe((result: CalibrationResult | null) => {
        ...
        if (result.type === CalibrationResultType.error) {
          const errorL10nKey =
            result.error instanceof OutOfRangeCalibrationError
  ...
  public setForm(form: ServoBindingForm): void {
    if (form !== this._form) {
      this._form = form;
      ...
      this._canCalibrate$ = form.controls.hubId.valueChanges.pipe(
        ...
        switchMap(() => {
          ...
          return this.store.select(
            BINDING_SERVO_EDIT_SELECTORS.canCalibrateServo({
