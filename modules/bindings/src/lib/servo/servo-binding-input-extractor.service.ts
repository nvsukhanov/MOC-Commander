...
export class ServoBindingInputExtractorService implements ITasksInputExtractor<ControlSchemeBindingType.Servo> {
  ...
  public extractInputs(
    ...
    const cw$ = this.inputExtractorService
      .extractInputResult(binding, globalInput$, controllersSettings$, binding.inputs[ServoBindingInputAction.Cw])
      .pipe(distinctUntilValueChanged());
    const ccw$ = this.inputExtractorService
      .extractInputResult(binding, globalInput$, controllersSettings$, binding.inputs[ServoBindingInputAction.Ccw])
      .pipe(distinctUntilValueChanged());
