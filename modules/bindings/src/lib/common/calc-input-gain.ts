...
function slopeFn(value: number, slopeGain: number, sharpness: number): number {
  return slopeGain * Math.pow(Math.abs(value), sharpness) * Math.sign(value) + (1 - slopeGain) * value;
}

export function logarithmicInputGain(value: number): number {
  return slopeFn(value, LOGARITHMIC_SLOPE_GAIN, LOGARITHMIC_SHARPNESS);
}

export function exponentialInputGain(value: number): number {
