...
export class TrainBindingFormBuilderService implements IBindingFormBuilder<TrainBindingForm> {
  ...
  public build(): TrainBindingForm {
    return this.formBuilder.group({
      ...
      levels: this.formBuilder.array<FormControl<number>>([this.commonFormControlsBuilder.speedLevelControl(0)], {
  ...
  public patchForm(form: TrainBindingForm, patch: ControlSchemeTrainBinding): void {
    form.patchValue(patch);
    form.controls.levels.clear();
    patch.levels.forEach((speed) => form.controls.levels.push(this.commonFormControlsBuilder.speedLevelControl(speed)));
