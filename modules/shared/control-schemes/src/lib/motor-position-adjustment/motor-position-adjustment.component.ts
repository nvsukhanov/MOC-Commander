...
@Component({
  ...
  imports: [MotorPositionAdjustmentControlsComponent, AsyncPipe],
...
export class MotorPositionAdjustmentComponent implements OnChanges, OnDestroy {
  ...
  public ngOnChanges(): void {
    ...
    this._canExecuteStep$ = this.store.select(
      ATTACHED_IO_PORT_MODE_INFO_SELECTORS.selectIsIoSupportInputMode({
        hubId: this.hubId,
        portId: this.portId,
        portModeName: PortModeName.position,
      }),
    );
    this._canGoToZero$ = this.store.select(
      ATTACHED_IO_PORT_MODE_INFO_SELECTORS.selectIsIoSupportInputMode({
        hubId: this.hubId,
        portId: this.portId,
  ...
  public onExecuteStep(stepDegrees: number): void {
    if (this.hubId === null || this.portId === null) {
      return;
    }
    this.subscription?.unsubscribe();
    const hub = this.hubStorageService.get(this.hubId);
    this.subscription = hub.motors
      .rotateByDegree(this.portId, stepDegrees, {
        speed: this.speed,
        power: this.power,
        endState: MotorServoEndState.hold,
      })
      .pipe(last(), delay(500), concatWith(hub.motors.startSpeed(this.portId, 0, { power: 0 })))
      .subscribe();
  }
  ...
  public onGoToZero(): void {
    ...
    this.subscription = this.store
      .select(ATTACHED_IO_PROPS_SELECTORS.selectMotorEncoderOffset({ hubId: this.hubId, portId: this.portId }))
      .pipe(
        take(1),
        switchMap((motorEncoderOffset) => {
          ...
          const hub = this.hubStorageService.get(this.hubId);
          return hub.motors
            .goToPosition(this.portId, -motorEncoderOffset, {
              speed: this.speed,
              power: this.power,
              endState: MotorServoEndState.hold,
            })
