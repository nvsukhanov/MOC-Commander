...
@Component({
  ...
  imports: [MatSelectModule, TranslocoPipe, ReactiveFormsModule, IoTypeToL10nKeyPipe, PortIdToPortNamePipe, AsyncPipe],
...
export class BindingControlSelectIoComponent implements OnChanges, OnDestroy {
  ...
  public trackIo(index: number, item: AttachedIoModel): string {
  ...
  public ngOnChanges(): void {
    ...
    if (this.portIdControl && this.hubIdControl) {
      this.formUpdateSubscription = this.hubIdControl.valueChanges
        .pipe(
          startWith(null),
