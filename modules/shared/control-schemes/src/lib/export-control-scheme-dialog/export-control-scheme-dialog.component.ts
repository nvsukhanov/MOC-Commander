...
@Component({
  ...
  imports: [MatButtonModule, MatDialogModule, TranslocoPipe, AsyncPipe],
  providers: [ControlSchemeCompressorService],
...
export class ExportControlSchemeDialogComponent {
  ...
  constructor(
    ...
    this.exportString$ = this.store
      .select(CONTROL_SCHEME_SELECTORS.selectScheme(this.data.name))
      .pipe(map((scheme) => (scheme ? this.compressor.compress(scheme) : '')));
  }

  public onCopy(): void {
    this.exportString$.pipe(take(1)).subscribe((exportString) => {
