...
export class BindingEditComponent {
  ...
  protected form = this.formBuilder.group({
    bindingType: this.formBuilder.control<ControlSchemeBindingType>(ControlSchemeBindingType.Speed, {
      nonNullable: true,
      validators: [Validators.required, AppValidators.isInEnum(ControlSchemeBindingType)],
    }),
  });
  ...
  constructor(private readonly formBuilder: FormBuilder) {
    this.bindingFormDirtyChange = merge(this.form.statusChanges, this.specificBindingDirty$).pipe(
      startWith(null),
  ...
  @Input()
  public set binding(binding: Partial<ControlSchemeBinding> | null) {
