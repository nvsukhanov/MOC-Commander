...
export class BindingEditDetailsRenderDirective implements OnDestroy {
  @Output('libCsBindingEditDetailsRenderBindingChange')
  ...
  constructor(
    ...
    @Inject(BINDING_DETAILS_EDIT_FORM_RENDERER_FACTORY)
    private readonly factory: IBindingDetailsEditFormRendererFactory,
  ) {
  ...
  public set binding(b: Partial<ControlSchemeBinding> | null) {
    this.rendererInstance.setBinding(b);
  }

  @Input('libCsBindingEditDetailsRenderBindingType')
  public set bindingType(b: ControlSchemeBindingType) {
