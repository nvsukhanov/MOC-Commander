...
export class ControllerProfileXbox360Service extends GamepadProfile {
  ...
  private readonly ids: ReadonlySet<string> = new Set(['Xbox 360 Controller (XInput STANDARD GAMEPAD)']);
  ...
  public controllerIdMatch(id: string): boolean {
    ...
    return (vendorAndProduct?.vendorId === this.vendorId && vendorAndProduct?.productId === this.productId) || this.ids.has(id);
  }
}
