...
export function transformRawInputValue(rawValue: number, settings?: GamepadAxisSettings | GamepadButtonSettings): number {
  if (!settings) {
    return truncateAndClampInputValue(rawValue);
  }
  const preparedRawValue = rawValue * (settings.invert ? -1 : 1);
  return truncateAndClampInputValue(applyActiveZoneToInputValue(preparedRawValue, settings.activeZoneStart, settings.activeZoneEnd));
}
