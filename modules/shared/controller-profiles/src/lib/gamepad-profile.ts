...
export abstract class GamepadProfile implements IControllerProfile<GamepadSettings> {
  ...
  public getAxisName$(inputId: string): Observable<string> {
    return this.axisNames[inputId] ?? this.translocoService.selectTranslate(this.genericGamepadL10nScopeKeyBuilder('axis'), { inputId });
  }

  @Memoize()
  public getButtonName$(inputId: string): Observable<string> {
    return this.buttonNames[inputId] ?? this.translocoService.selectTranslate(this.genericGamepadL10nScopeKeyBuilder('button'), { inputId });
  }
  ...
  protected getTranslation(key: string): Observable<string> {
