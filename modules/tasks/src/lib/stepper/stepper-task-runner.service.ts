...
export class StepperTaskRunnerService implements ITaskRunner<TaskType.Stepper> {
  public runTask(hub: IHub, task: PortCommandTask<TaskType.Stepper>): Observable<PortCommandExecutionStatus> {
    return hub.motors
      .rotateByDegree(task.portId, task.payload.degree * (task.payload.action === StepperBindingInputAction.Cw ? 1 : -1), {
