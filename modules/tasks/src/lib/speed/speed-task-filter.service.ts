...
export class SpeedTaskFilterService implements ITaskFilter<TaskType.Speed> {
    constructor(
        private readonly hashBuilder: TaskHashBuilderService
    ) {
    }
    ...
    ): task is PortCommandTask<TaskType.Speed> {
    ...
    private buildCombinedTask(
        tasks: Array<PortCommandTask<TaskType.Speed>>
    ): PortCommandTask<TaskType.Speed> {
        if (!tasks.length) {
